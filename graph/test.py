...
dataset = tf.data.TFRecordDataset('data_testdata.tfrecord').map(lambda x: tf.io.parse_tensor(x, tf.float32))
...
response = requests.post('http://10.106.18.124/seldon/models/locus-graph/api/v1.0/predictions', json = request_body)
